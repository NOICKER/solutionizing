...
// The exact loading block before the main return
...
let returnIdx = src.indexOf(loadingBlock);

if (returnIdx === -1) {
  // Let's try a regex for the loading block
  const loadingMatch = src.match(/if\s*\(isLoading\)\s*\{\s*return\s*\([\s\S]*?<WizardStepSkeleton step=\{1\} \/>[\s\S]*?\)\s*\}/);
  if (!loadingMatch) {
    throw new Error('Could not find loading block');
  }
  returnIdx = src.indexOf(loadingMatch[0]) + loadingMatch[0].length;
} else {
  returnIdx += loadingBlock.length;
}

// Find the function body *before* the main return
let regionA = src.substring(0, returnIdx);

// Find the closing of MissionWizardPage
...
let regionB_raw = src.substring(returnIdx, closingFnMatch);
let regionC = src.substring(closingFnMatch);

// ── 2. Clean up Region A ──────────────────────────────────────────────

// 2a. Change `const [step, setStep] = useState(1)` to `const [activeStage, setActiveStage] = useState<'brief' | 'setup' | 'questions' | 'review'>('brief')`
regionA = regionA.replace(
  `const [step, setStep] = useState(1)`,
  `const [activeStage, setActiveStage] = useState<'brief' | 'setup' | 'questions' | 'review'>('brief')`
);

// 2b. Remove the browser history step useEffects
regionA = regionA.replace(
...
// 2c. Remove handleNext function
regionA = regionA.replace(
  /\s+function handleNext\(\) \{[\s\S]*?const nextStep = Math\.min\(4, step \+ 1\)\n\s+if \(nextStep !== step\) \{\n\s+if \(typeof window !== 'undefined'\) \{\n\s+window\.history\.pushState\(\{ step: nextStep \}, ''\)\n\s+\}\n\s+setStep\(nextStep\)\n\s+\}\n\s+\}/,
  ''
);

// 2d. Remove handleBack function
regionA = regionA.replace(
  /\s+function handleBack\(\) \{[\s\S]*?setStep\(\(current\) => current - 1\)\n\s+\}\n\s+\}/,
  ''
);

// 2e. Fix handleSave — replace step references with stage references
regionA = regionA.replace(
  /const targetStep = getStepForFieldKey\(firstErrorKey\)\n\s+if \(typeof window !== 'undefined'\) \{\n\s+window\.history\.pushState\(\{ step: targetStep \}, ''\)\n\s+\}\n\s+setStep\(targetStep\)/,
  `const targetStage = getStageForFieldKey(firstErrorKey)\n      setActiveStage(targetStage)`
);

// 2f. Replace getStepForFieldKey with getStageForFieldKey
regionA = regionA.replace(
  /function getStepForFieldKey\(fieldKey: string\) \{\n\s+if \(fieldKey === 'title' \|\| fieldKey === 'goal'\) \{\n\s+return 1\n\s+\}\n\n\s+if \(fieldKey === 'assets' \|\| fieldKey\.startsWith\('asset-'\)\) \{\n\s+return 2\n\s+\}\n\n\s+return 3\n\s+\}/,
...
// ── 4. Remove old functions that are no longer needed ──────────────────

// Remove StepIndicator function
regionA = regionA.replace(
  /function StepIndicator\(\{ step \}: \{ step: number \}\) \{[\s\S]*?\}\n\}/,
  ''
);

// Remove renderStepNavigation function
regionA = regionA.replace(
  /\s+function renderStepNavigation\(position: 'top' \| 'bottom'\) \{[\s\S]*?\}\n\s+\}/,
  ''
);

// ── 5. Update the loading component ──────────────────────────────────
regionC = regionC.replace(
  /function MissionWizardPageLoading\(\) \{[\s\S]*?\}\n\}/,
...
const finalContent = regionA + newReturn + '\n\n' + regionC;

fs.writeFileSync(srcPath, finalContent, 'utf8');
console.log('✅ Successfully transformed MissionWizardPage into stage-based builder!');
