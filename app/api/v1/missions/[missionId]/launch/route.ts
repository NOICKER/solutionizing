...
export async function POST(
  ...
  try {
    ...
    // Queue tester assignment as a non-fatal side-effect.
    // The mission is already ACTIVE — assignment can be retried separately.
    try {
      await assignmentQueue.add('assign', { missionId: mission.id })
    } catch (queueErr) {
      console.error(
        `[MissionLaunch] Failed to enqueue tester assignment for mission ${mission.id}. ` +
          'Assignment must be retried manually.',
        queueErr
      )
    }
