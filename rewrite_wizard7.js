...
const searchStr = '<div className="min-h-screen bg-[var(--bg)] pb-12">';

let split1 = content.split(searchStr);

if (split1.length < 2) {
  console.log("Could not find start of render block.");
} else {
  let topPart = split1[0];
  let bottomPart = searchStr + split1.slice(1).join(searchStr);
  ...
  if (step1Index > -1) {
    ...
    if (stepIndIndex > -1) {
      ...
      const col1Start = `
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 h-[calc(100vh-120px)] items-start mt-6">
...
`;
      ...
      // Since the indentation of `<div className="space-y-8 rounded-[12px]...` can be 2, 4, 8 spaces, let's use a regex replace for step boundaries!
